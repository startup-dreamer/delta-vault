...
/**
 * @title DeltaVaultClient
 * @dev API consumer contract to get token price from oracle
 */

contract DeltaVaultClient is ChainlinkClient, ConfirmedOwner {
    ...
    mapping(address => TokenPrice) private tokenToPrice;
     ...
     */
    function getTokenPrice(address _tokenAddress) public returns(TokenPrice memory) {
        if(tokenToPrice[_tokenAddress].deadline >  block.timestamp) {
            return tokenToPrice[_tokenAddress];
        }
    ...
    /**
     * @dev Sets the address of the DeltaVault contract.
     * @param _deltaVaultAddress The address of the DeltaVault contract.
     * Requirements:
     * - The caller must be the owner of the contract.
     */
    function setDeltaVaultAddress(address _deltaVaultAddress) external onlyOwner {
        deltaVaultAddress = _deltaVaultAddress;
    }
    /**
     * @dev Requests the price of a token from the oracle.
    ...
    function requestPrice(address _tokenAddress) public ownerOrDeltaVaultAddress {
